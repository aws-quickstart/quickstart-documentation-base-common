...
+
. Check the AWS Region that’s displayed in the upper-right corner of the navigation bar, and change it if necessary. This is where you're building the network infrastructure. The template launches in the {default_deployment_region} Region by default.
. On the *Create stack* page, keep the default setting for the template URL, and then choose *Next*.
...
. Use the values displayed in the *Outputs* tab for the stack to view the created resources.
